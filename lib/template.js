 ...
 * in order to process a single message. Either `command` or `lambda` must be
 * specified.
 * @param {String} [options.lambda] - if defined, it must be the Logical name
 * of a lambda function that is defined by this stack. Specifying this property
 * puts the watchbot system into lambda-mode, where each message is processed
 * by the provided lambda function. Either `command` or `lambda` must be
 * specified.
 ...
 * notifications when processing fails. Either a notificationEmail or a
 * notificationTopic must be provided.
...
module.exports = (options = {}) => {
  ['service', 'serviceVersion', 'cluster'].forEach((required) => {
    if (!options[required]) throw new Error(`options.${required} is required`);
  });

  if (!options.command && !options.lambda)
    throw new Error('options.command or options.lambda must be specified');

  if (options.command && options.lambda)
    throw new Error('options.command and options.lambda cannot both be specified');
  ...
  if (options.lambda)
    Resources[prefixed('Role')].Properties.Policies.push({
      PolicyName: cf.join([cf.stackName, '-lambda-invocation']),
      PolicyDocument: {
        Statement: [
          {
            Effect: 'Allow',
            Action: 'lambda:InvokeFunction',
            Resource: cf.getAtt(options.lambda, 'Arn')
          }
        ]
      }
    });
      ...
      ContainerDefinitions: [
        {
          ...
          Command: options.lambda
            ? ['watchbot', 'lambda-listen', cf.ref(options.lambda), options.maxSize]
            : ['watchbot', 'listen', `${options.command}`],
          Ulimits: [
            {
              Name: 'nofile',
              SoftLimit: 10240,
  ...
  Resources[prefixed('ScalingTarget')] = {
    ...
    Properties: {
      ...
      MaxCapacity: options.lambda ? 1 : options.maxSize,
